...
async function main() {
  ...
  const arFile = path.join(basePath, 'ar.json')

  const enData = JSON.parse(fs.readFileSync(enFile, 'utf-8'))
  const arData = fs.existsSync(arFile) ? JSON.parse(fs.readFileSync(arFile, 'utf-8')) : {}
  ...
  fs.writeFileSync(arFile, JSON.stringify(merged, null, 2), 'utf-8')
  console.log(`✅ Translation complete. Updated file: ${arFile}`)
}
